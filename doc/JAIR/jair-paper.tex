...
formula $\theta$ over the variables $\varset \cup \exvarset(\theta)$, we say that $\phi$ is {\em
  equivalent over $\varset$} to $\theta$, written
$\phi \eequiv \theta$,
...
\ref{fig:eg4:all}(D) that these two clauses form a RUP proof for the clause
...
that there is no total assignment $\assign$ that falsifies this clause but extends to $\eassign$ such that $\eassign(s_{10}) = 1$.
...
clauses (e.g., clause 36 asserting that $\eassign(s_{10}) = 1$) or
