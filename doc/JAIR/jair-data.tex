...
\begin{itemize}
\item Of the 122 benchmarks, 100 were completed using the hybrid approach within 10,000 seconds.
\item Of the 12 that did not completed, able to perform one-sided verification for 8 of them.
\item Leaves 4 (+ 1 overflow case) for which no form of validation succeeded
\end{itemize}
...
\begin{tikzpicture}[scale = 0.70]
  \begin{axis}[mark options={scale=0.55},grid=both, grid style={black!10}, ymode=log,
      legend style={at={(0.30,0.96)}},
      legend cell align={left},
                              x post scale=1.8, y post scale=2.0,
                              ...
                              xlabel={D4 runtime (seconds)}, ylabel={Toolchain runtime (seconds)},
            ]

    \input{data-formatted/sub10M-seconds-hybrid}
    \legend{
      \scriptsize \textsf{Hybrid},
    }

    \input{data-formatted/sub10M-seconds-failures-hybrid}
