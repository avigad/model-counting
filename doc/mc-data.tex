...
\begin{enumerate}
   ...
   \item Generated POG representations from dDNNF graphs
     ...
     \item Measure complexity by number of defining clauses $D$
       ...
       \end{itemize}
     \end{itemize}
   \item Ran program {\sc d2p} to generate CPOG file for maximum of 10,000 seconds
     \begin{itemize}
...
\section*{Time to Generate and Validate CPOG}
...
\input{figure-clauses-d4-result}
...
\item Monolithic completed  69,timed out on 14.
\item Of the 69 completed, 19 were faster with monolithic, 50 were slower (min ratio = 0.013, max ratio = 18.97, hmean = 0.41)
...
Run same 83 benchmarks used to compare monolithic vs. structured mode.
Incorporate time to run D4 into evaluation, since differs according to
how D4 runs.

Runtime results:
\begin{itemize}
\item Monolithic without preprocessing completes 68  of then with an overall time less than 1000 seconds.  (One less than when only consider generation and checking).
\item Monolithic with prepreprocessing completes these plus one more (not the same as previously excluded)
\item Of the 68, 53 run faster with preprocessing and 15 run slower (mininum ratio 0.04, maximum ratio 1.65, harmonic mean 0.38)
\item Shifts only slightly when don't count D4 time.  Both complete 69 benchmarks. 56 faster, 13 slower (minimum ratio 0.04, maximum ratio 1.49, harmonic mean 0.37)
\end{itemize}

Clause results:
\begin{itemize}
\item Consider the 68 cases that completed for both with overall time less than 1000 seconds.
\item 27 had more clauses without preprocessing, 19 were identical, and 22 had less.
\item Min = 0.43, Max = 4.57, Harmonic mean = 0.98
\end{itemize}
