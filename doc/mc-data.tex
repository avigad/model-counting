...
\begin{enumerate}
   ...
   \item Ran program {\sc d2p} to generate CRAT file for maximum of 10,000 seconds
     ...
     \item Exceeded clause limit for 1 instance
...
\begin{tikzpicture}[scale = 1.00]
  \begin{axis}[mark options={scale=0.55},grid=both, grid style={black!10}, ymode=log, ymin=100, ymax=10000000000,
      legend style={at={(0.30,0.98)}},
      legend cell align={left},
                              x post scale=2.0, y post scale=2.0,
                              xmode=log, xmin=001,xmax=10000,
...
\begin{itemize}
\item Proof Clauses: Min 554, Median 1,719,245, Average 45,001,945, Max 1,719,245
\item Ratio to number of defining clauses: Min 1.54, Harmonic Mean 3.13, Max 6073
\end{itemize}
...
\item Ratio to time for D4: Min 0.50, Harmonic Mean 5.5, Max 457.7
\end{itemize}
...
\centering{%
\begin{tikzpicture}[scale = 1.00]
  \begin{axis}[mark options={scale=0.55},grid=both, grid style={black!10}, ymode=log, ymin=0.01, ymax=100000,
                              x post scale=2.0, y post scale=2.2,
                              xmode=log,xmin=0.01,xmax=10000,
                              xtick={0.01, 0.1,1.0,10,100,1000,10000}, xticklabels={0.01, 0.1, 1.0, 10, 100, {1,000}, {10,000}},
...
\caption{Proof Clauses when Disable Options}
\label{fig:defining:unoptimized}
...
performed was limited to $10^7$.  We then made three runs---with each
of the two optimizations disabled individually and in combination.
Figure \ref{fig:defining:unoptimized} illustrates the effect of the
two optimizations.  Each set of points shows the number of clauses
when both optimizations are performed for the $X$ value and the number
with one or both of these optimizations disabled as the $Y$ value.
...
\end{description}
Put another way: None of the 80 benchmarks require more than $10^7$
...
either, 25 rise above that threshold and 2 even exceed $10^8$ proof
clauses.
