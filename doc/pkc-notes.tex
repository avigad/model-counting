...
%%Let $\pure(\phi)$ denote the set of all pure literals in $\phi$, and $\pure_{\pvarset}(\phi) = \{ \lit \in \pure(\phi) \mid  \dependencyset(\lit) \subsete%%q \pvarset \}$.
%%If $\pure_{\pvarset}(\phi) \not = \emptyset$, then $\phi$ can be reduced as
%% $\phi \leftarrow \cfilter(\phi, \pure_{\pvarset})$.  Eliminating clauses can cause other literals to become pure, and so this reduction should be iterate%%d until no pure literals remain.
\end{itemize}
