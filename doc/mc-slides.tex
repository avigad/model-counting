...
\begin{itemize}
  \bitem Problem variables $X$
  \bitem Tseitin variables $Z$
\end{itemize}
    \item For any assignment $\alpha$ to variables in $X$, there is a
      unique assignment $\alpha^Z$ to the variables in $X \cup Z$ satisfying
      all clauses containing variables in $Z$.
  \end{itemize}
  \medskip
...
\begin{itemize} 
\item For any assignment $\alpha$, with extensions $\alpha^{Z_1}$ and $\alpha^{Z_2}$:
  \begin{itemize}
  \item[] $\alpha^{Z_1}$ satisfies $\psi_1$ if and only if $\alpha^{Z_2}$ satisfies $\psi_2$
  \end{itemize}
\end{itemize}      

\medskip
{\bf CPOG Proof Objective}
\vskip -1.5em
           \begin{eqnarray*}
...
\vskip -1.0em
\begin{eqnarray*}
  \phi_I(X, \emptyset) & \timply & \theta_P(X, \emptyset)
\end{eqnarray*}
%\vskip -0.5em
...
\vskip -1.0em
\begin{eqnarray*}
  \theta_P(X, Z) & \timply & \phi_I(X, \emptyset)
\end{eqnarray*}
\vskip -0.5em
\begin{itemize}
...
\begin{itemize}
\bitem {\em When extended, any assignment falsifying the input clause causes  the POG to evaluate to false}
\end{itemize}
