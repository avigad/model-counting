...
\frame{
\frametitle{Automated Reasoning Programs}
...
\begin{itemize}
\item Only need to prove individual executions, not entire program
\item Can have bugs in tool but still trust result
\only<2->{
\item Can we trust the checker?
  \begin{itemize}
  \bitem Simple algorithms and implementation
  \bitem Possibly formally verified
  \end{itemize}
}
\end{itemize}
...
\draw[line width=3pt, -latex] (topl) -- (sat);
...
\draw[line width=3pt, -latex] (botl) --(unsat);
}
...
\draw[line width=3pt, -latex] (sat.north) -- (check.south);
}
...
\only<2->{
  {\bf Questions}
  \begin{itemize}
  \item {\em How do I know the generated representation is logically equivalent to the input formula?}
  \item {\em How do I know if the computed query values are correct?}
  \end{itemize}
} %% only
\end{frame}
...
\medskip
\begin{itemize}
\bitem {\footnotesize Joint work with Wojciech Nawrocki, Jeremy Avigad, and Marijn Heule}
\end{itemize}
\medskip
\centering{
...
\begin{frame}
\frametitle{Trusting the Trusted Code}
...
\medskip
{\bf Within Lean 4 Proof Framework \reference{DemUlr-2021}}
\begin{itemize}
\item Soundness of proof system
  \begin{itemize}
    \bitem Helped us identify some weaknesses in our proof rules
  \end{itemize}
\item Verified checker
  \begin{itemize}
  \bitem Around $6\times$ slower than one implemented in C
%  \bitem Same asymptotic performance
  \end{itemize}
\item Ring Evaluator: Over rationals
\end{itemize}
\end{minipage}
\end{frame}
...
\frametitle{CPOG Declaration + Proof}
           ...
           \begin{itemize}
             ...
             \item Children indicated by literals
               \begin{itemize}
                 \bitem Positive or negated arguments
                 \bitem Input variables or results from other operation
               \end{itemize}
...
\begin{frame}
\frametitle{CPOG Proof Structure}

{\bf Forward Implication Proof}
\vskip -1.0em
\begin{eqnarray*}
...
\vskip -0.5em
\begin{itemize}
\item Add clauses by resolution
\item Terminating with unit clause $[r]$
\item {\em Any assignment satisfying $\phi_I$  causes the POG formula to evaluate to true}
\end{itemize}
...
\vskip -0.5em
\begin{itemize}
\item Delete clauses by resolution
  \begin{itemize}
    \bitem Deleted clause implied by remaining ones
  \end{itemize}
\item Including each of the input clauses
\item {\em Any assignment falsifying the input clause causes the POG formula to evaluate to false}
\end{itemize}
...
\begin{frame}
\frametitle{Experimental Results: CPOG Generation and Checking}
\bigskip
...
\begin{itemize}
\bitem 180 benchmark files from 2022 model checking competitions
\bitem D4 completed 124 with 4000-second time limit
\bitem Generated complete proofs for 108 with 10,000-second time limit
\bitem Reverse implications for 9
\bitem No proofs for 7
\end{itemize}
\end{minipage}
\end{frame}
...
\begin{itemize}
\bitem 108 problems fully verified
\bitem CPOG files up to 160~GB
\bitem Reverse implications for 9
\bitem No proofs for 7
\end{itemize}
\end{minipage}
