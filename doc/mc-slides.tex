...
\item Assignment $\alpha$ to $X$
\item Defining clauses induce unique extension $\alpha^{*}$ to $X \cup Z$
\item $\alpha$ satisfies $\phi_I$ if and only if $\alpha^{*}(r) = 1$
\end{itemize}
...
{\bf Proof Methodology}
\begin{itemize}
\item Transform $\phi_I$ to $\theta_P$ by series of equivalence preserving proof steps
\end{itemize}
...
\begin{itemize}
\bitem {\em For any  assignment falsifying the input clause, extending it to satisfy the defining clauses causes the POG to evaluate to false}
\end{itemize}
