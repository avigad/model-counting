...
\item $\Delta(\phi, w)$ gives {\em density} of function
  \begin{itemize}
...
\item {\em Weighted Counter}: Compute standard or weighted model count
\end{itemize}
...
\midrule
  ...
  \rtext{4} & \only<2-3>{\gtext{\texttt{-3 -1}}} \only<4>{\gtext{\texttt{\emph{-3 -1}}}} &   \\
...
%% \begin{itemize}
%% \bitem {\em For any  assignment $\alpha$ falsifying the input clause, its extension $\alpha^{*}$ yields $\alpha^{*}(r) = 0$}.
%% \end{itemize}
...
\begin{tikzpicture}[scale = 0.52]
  \begin{axis}[mark options={scale=0.55},grid=both, grid style={black!10}, ymode=log,
      ...
      legend cell align={left},
                              ...
                              xlabel={D4 runtime (seconds)}, ylabel={CPOG generation, checking, and weighted count  (seconds)},
