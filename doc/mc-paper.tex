...
also satisfies $\phi_2$ by setting $x_2$ to 1.
...
commandline argument to \dtrim{} that enforces this
restriction.\footnote{Available at
\url{https://github.com/marijnheule/drat-trim/tree/develop}.}  This weakness, however, illustrates the general challenge of
...
We can now define and compare the two related representations:
\begin{itemize}
\item A det-DNNF graph is a POG with negation applied only to the variables.
\item A dec-DNNF graph is det-DNNF graph with the further restriction that any
  sum node $\nodeu$ has have exactly two children $\nodeu_1$ and $\nodeu_0$, and for these there is a \emph{decision variable} $x$ such that
...
\dfour{}.  The right-hand side shows the total number of clauses in the CPOG file versus the number of defining clauses.
...
knowledge compilation, the \cdfour{} toolchain completed for 106
benchmarks, while the CPOG toolchain completed for just 82.  We found
...
the recursion through lemmas.  On the other hand, neither of these
