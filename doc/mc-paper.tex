...
Computing certain properties of Boolean formulas, such as their weighted or unweighted model count,
is intractable on general representations. It can become tractable when formulas are expressed in a
special form, such as d-DNNF. \emph{Knowledge compilation} is the process of converting a formula
into such a form. Existing knowledge compilers provide no guarantee that their output correctly
represents the original formula, and therefore they cannot validate the resulting model counts.
...
We have designed CRAT, a proof framework that can prove the equivalence of a
...
\dfour{}, as well as checkable CRAT proofs certifying that the output POGs
...
competition. Additionally, we have developed a formally verified CRAT
...
generates a d-DNNF representation, and the {\em proof
  generator} uses this to generate a CRAT file.
The {\em proof checker} verifies the equivalence of the CNF and CRAT representations.
...
Sections~\ref{sect:crat} and \ref{section:formally-verified-toolchain}.
...
$\inputformula \ifandonlyif \exists Z\,\theta$. We write (subscripted) $\theta$ for
clausal encoding formulas possibly using extension variables, and (subscripted) $\phi$ for formulas
that do not.
...
benchmark problems are available at \url{https://github.com/rebryant/crat}}.  The proof generator is the same in both
