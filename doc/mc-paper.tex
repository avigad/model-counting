...
compiler D4, as well as independently checkable proofs that the
...
generated by D4 for formulas
from a recent model counting competition. Moreover, we
present a checker that has been formally verified
in the Lean~4 proof assistant, thereby creating the
first formally verified toolchain for weighted and unweighted model
counting.
...
\item[Partitioned Product] $\rep\left(\Pand_{i=1,k} \phi_i, w \right) = \prod_{i=1,k} \rep(\phi_i, w)$
\item[Partitioned Sum] $\rep\left(\Por_{i=1,k} \phi_i, w\right) = \sum_{i=1,k} \rep(\phi_i, w)$
\end{description}
\end{proposition}
...
graph.  A RUP proof for input clause $C$ effectively proves that any
total assignment $\assign$ that does not satisfy $C$ will yield
$\assign(r) = 0$.  It starts with the set of literals
$\{ \obar{\lit} | \lit \in C\}$, describing the required condition for
...
Working upward through the graph, each node $\nodeu$ is {\em marked} when
the collected set of literals force $u$ to evaluate to $0$.  The
...
added to the hints.  Marking sum node $\nodeu$ requires that both of
is children $\nodeu_1$ and $\nodeu_2$ be marked.  The first defining
clause for this node (see Table~\ref{tab:defining}(B)) will then be added.  At the very end, the program
...
each clause deletion requires a single RUP step, where 
the number of hints will be less than the number of
nodes in the POG\@.  
...
A key feature of both is that they do not require any changes to the
...
having many---tens or even hundreds---of literal as children.  As described
earlier, the standard approach for each child literal $\lit \in \L$ is to first attempt BCP, but calling a SAT solver
on the formula  $\simplify{\psi}{\passign \cup \{\obar{\lit}\}}$.

An alternative approach is to collect those literals $\lit_1, \lit_2,
\ldots, \lit_m$ that cannot be validated by BCP and then define a
product node $\nodev$ having these literals as children.  The goal
then becomes to prove that this product must yield 1 for extension
variable $v$.  This can be done by contradiction using a SAT solver.
In particular, looking at the defining clauses for $\nodev$ (Table
\ref{tab:defining}(A)), we can see that assigning 0 to $v$ will cause
  the first defining clause to simplify to $C_p = \obar{lit}_1 \lor
  \obar{\lit}_2 \lor \cdots \lor \obar{\lit}_m$, and for the remaining
  clauses to become tautologies.  The formula
  $\simplify{\psi}{\passign} \cup \{ C_p \}$ should therefore be
  unsatisfiable.  The proof steps and hints generated by the SAT
  solver can mapped back to the input clauses, plus the defining
  clauses for $\nodev$.
...
clauses arising from a partial assignment to the literals matches that
of a previous traversal.  When this d-DNNF node is translated into POG
node $\nodeu$, the proof generator can assume (and later check), that
...
maintain the graph structure of the POG, invoking $\validate$ on each
node at most once.

The challenge for implementing this strategy is to find a way to
represent the clauses in a simplified formula $\gamma$.  Some may be
unalterered input clauses, and these can be used directly.  Others,
...
the appropriate clauses.  Imagine that there is an {\em argument} clause
$C \in \gamma$ with $C = \lit_1 \lor \lit_2 \lor \cdots \lit_k$.  If we
define product node $\nodev$ with arguments
...
Suppose for every clause $C_j \in \gamma$ that does not correspond to
an input clause, we generate a synthetic clause $C'_j$ with guard literal
$\obar{v}_j$, for $1 \leq j \leq m$.  Let $\gamma$ be the formula where each clause $C_j$ is replaced by synthetic clause $C'_j$,
and
...
Invoking $\validate(\nodeu, \lassign, \gamma)$
 ...
 showing that any total assignment $\assign$ that activates the synthetic clauses will have $\assign(u)$.
...
we assert the clause
$\obar{v}_j \lor \bigvee_{\lit \in \passign} \obar{\lit}$
to show that synthetic clause $C_j$ will be activated for $1 \leq j \leq m$.
