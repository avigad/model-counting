...
tools handle all but the largest d-DNNF graphs generated
...
By contrast, our proofs construct a new propositional formula, and we must verify
%both
that it
...
\item For a single nonterminal child ($k = 1$), we recursively call
  $\validate \left(\nodeu_1, \passign \cup L, \psi\right)$.
\item For multiple nonterminal children ($k > 1$),
  ...
  We recursively call
  $\validate \left(\nodeu_i, \passign \cup L, \psi_i\right)$.
\end{enumerate}
