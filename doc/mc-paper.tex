...
of each simplified clause $C$, i.e., which of the original clauses simplified to become $C$.
More formally, for $C \in \simplify{\phi}{\passign}$, we let $\expand(C)$ denote
those clauses $C' \in \phi$, such that
$C' \subseteq C \cup \bigcup_{\lit \in \passign} \obar{\lit}$.
We then extend the definition of $\expand$ to any simplified formula
$\psi$ as $\expand(\psi) = \bigcup_{C \in \psi} \expand(C)$.
...
\item if $u$ is a literal $\lit$, then the formula
  ...
  $C = \lit$, such that $\expand{C}
  this clause.  That means that every other literal $\lit' \in C$ must
  have $\passign(\lit') = 0$, and therefore this input clause can
...
\item If $u$ is a disjunction node with children $u_1$ and $u_0$,
  ...
  $u \lor \obar{x} \lor \bigvee_{\lit \in \passign} \obar{l}$.
Similarly, either $u_0  = \obar{x}$ or $u_0$ is a conjunction node containing $\obar{x}$ as
 ...
 with the partial assignment $\passign \cup \{ \obar{x} \}$.
  This will yield a sequence of proof steps
  $u \lor x \lor \bigvee_{\lit \in \passign} \obar{l}$.
...
\item If $u$ is a conjunction node, then we can divide its children
  into literal nodes and nonterminal nodes.
  \begin{enumerate}
    \item For each literal child
  $\lit$, we must prove that any total assignment $\alpha$, such that
  $\passign \subset \alpha$ must have $\alpha(\lit) = 1$.  In some
  cases, this can be done by simple Boolean constraint propagation.
  In other cases, we must prove that the formula
  $\simplify{\phi}{\passign \cap \{\obar{l}\}} is unsatisfiable.  We
...
\item if there are multiple nonterminal children $u_1, u_2, \ldots,
  u_k$, it must be the case that the clauses in
  ...
  literals.  Moreover, each clause $C$ in each subset $\psi_i$ must be
  the simplified version of one or more input clauses $C'$ in the
  original formula $\phi$.  We define $\phi_i$ to consist of all such
  clauses $C'$ for each $C \in \psi_i$.

We do so using a union-find algorithm and match the variables in each partition
    $\phi_i$ to the POG node $u_i$ containing literals of the variables in
    $\dependencyset(\phi_i)$.
    We then recursively generate target clauses for nodes $u_1, u_2, \ldots, u_k$ 
