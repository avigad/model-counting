...
decomposable, negation normal form (d-DNNF)\@.
...
the decision-DNNF graphs (a restricted form of d-DNNF)
...
Darwiche~\cite{darwiche:aaai:2002,darwiche:ecai:2004} (as well as the more restrictive {\em decision-DNNF} representation~\cite{oztok:cp:2014})
 represents a
...
We establish a set of conditions that, when satisified by a CPOG file, guarantee that it
...
The {\em proof checker} verifies the equivalence of the CNF and CPOG representations.
The {\em ring evaluator} computes
...
To ensure soundness of the abstract CPOG proof system, as well as correctness of its
...
Running these two programs on a particular CPOG file gives assurance that the proof
...
We evaluate our toolchain using benchmark
formulas from the 2022 standard and weighted model competitions.  Our
tools handle all but the largest decision-DNNF graphs generated
by \dfour{}.  We also measure the benefits of several optimizations as
well as the relative performance of the verified checker with one
designed for high performance and capacity.
...
Capelli et al.\ developed a version of the \dfour{} knowledge compiler
that generates a certificate in a proof system that is itself based on
decision-DNNF~\cite{capelli:sat:2019,capelli:aaai:2021}.  Their
modified version of \dfour{} generates additional annotations to the
compiled representation to enable checking for equivalence with the
input formula.  They must also track cases where the knowledge
compiler detects that it can reuse a previously generated subgraph,
yielding a DAG rather than a tree representation.  Their proof checker
then must perform a number of checks that we incorporate into our
proof generator.  Our framework has the advantage of being independent
from the means by which the proofs are generated with proof checking
based on very elementary clausal reasoning steps.
...
based on the algorithms commonly used by model counters.  They present
a series of lemmas and a theorem stating that their checking framework
is sound.  They also present experimental results for cases where they
modify existing model counters to generate proof traces, and where
they generate proof traces from decision-DNNF graphs.
...
equivalent to the input formula.  We can then validate a standard or
weighted count from that representation.
%% Their tool
...
\begin{definition}[Boolean Formulas]
  ...
  \item For formulas $\phi_1, \phi_2, \ldots, \phi_k$, their {\em product} $\phi = \bigwedge_{1 \leq i \leq k} \phi_i$ is a Boolean formula, with
      $\dependencyset(\phi) = \bigcup_{1 \leq i \leq k} \dependencyset(\phi_i)$ and
      $\modelset(\phi) = \bigcap_{1 \leq i \leq k} \modelset(\phi_i)$.
  \item For formulas $\phi_1, \phi_2, \ldots, \phi_k$, their {\em sum} $\phi = \bigvee_{1 \leq i \leq k} \phi_i$ is a Boolean formula, with
      $\dependencyset(\phi) = \bigcup_{1 \leq i \leq k} \dependencyset(\phi_i)$ and
...
\end{definition}
  ...
  formula is in {\em conjunctive normal form} (CNF) when i) it is in
...
\label{def:ring_evaluation}
  ...
  For Boolean formula $\phi$ and ring weight function $w$, the {\em ring evaluation problem} computes
  \begin{eqnarray}
...
Every POG has a designated {\em root node} $\noder$.
...
POGs generalize decision-DNNF graphs in two ways:
\begin{itemize}
...
\item As with d-DNNF~\cite{darwiche:aaai:2002,darwiche:jair:2002}, they allow arbitrary arguments to a $\por$ operation, as long as
  these have disjoint models.  By contrast, each
  sum node $\nodeu$ in a decision-DNNF must have two children $\nodeu_1$ and $\nodeu_0$, and for these there must be a {\em decision variable} $x$ such that
...
\end{itemize}
The generalizations encompassed by POGs have also been referred to as {\em deterministic decomposable circuits} (d-Ds)~\cite{monet:amw:2018}.
Our current proof generator only works for knowledge compilers
generating decision-DNNF representations, but these generalizations
...
a set of {\em active} clauses $\theta$, i.e., those that
...
{\em antecedent} clauses, such that each $D_i$ is in $\theta$.
A RUP step
...
\section{The CPOG POG Representation and Proof System}
\label{sect:cpog}
...
%% \begin{itemize}
%% \item The {\em proof generator} produces a CPOG file that both defines the POG and gives a proof that the POG is logically equivalent to the input formula.
%% As the figure indicates, the generator starts with the output of another knowledge compiler, such as one generating a decision-DNNF representation of the formula.
...
\begin{itemize}
  ...
  Declaring a node $\nodeu$ implicitly adds an {\em extension} variable $u$ and a set of {\em defining} clauses $\theta_{u}$
...
into a POG formula.  Validating all of these steps serves to prove that
...
falsify $\phi_\noder$.  Deleting all but the final asserted clause and all input clauses provides a {\em reverse implication} proof
that
$\modelset(\phi_\noder) \subseteq \modelset(\inputformula)$.

Appendix~\ref{app:cpog:example} shows the CPOG description for
...
\section{Generating CPOG from decision-DNNF}
...
%%strategy to generate a decision-DNNF representation.
A decision-DNNF
...
\item If $\nodeu$ is a sum node with children $\nodeu_1$ and $\nodeu_0$,
  then, since the node originated from a decision-DNNF graph, there must be
...
on an algorithm to test for clausal entailment in d-DNNF graphs~\cite{capelli:sat:2019,darwiche:jair:2002}.  The
...
results (Appendix~\ref{app:experiments}) show that these
...
Figure~\ref{fig:chain}\footnote{All tools, along with a set of 
benchmark problems are available at \url{https://github.com/rebryant/cpog}}.  The proof generator is the same in both
...
Appendix~\ref{app:experiments} provides a more complete description. For our
...
any errors in the decision-DNNF graphs generated by \dfour{}.
...
When this decision-DNNF node is translated into POG
...
was able to generate decision-DNNF representations for 124 of these, and it timed out on the
rest.  Our proof generator was able to convert all of the generated
decision-DNNF graphs into POGs and determine how many defining clauses they
...
For each of the 124 decision-DNNF graphs generated by \dfour{}, we ran
...
\input{figure-mice-cpog} The detailed results are shown in
