...
efficient checker that can handle large proofs. Our tool chain
...
that their checking framework is sound.  They also
...
experimental evaluation in Appendix~\ref{app:experiments}.  Our
...
\textstyle
\scalebox{1.2}{\rep}\left(\Pand_{i=1,k} \phi_i,\; w \right) &=& \prod_{i=1,k} \rep(\phi_i, w) \label{eqn:ring:product} \\
\textstyle
\scalebox{1.2}{\rep}\left(\Por_{i=1,k} \phi_i,\; w\right) &=& \sum_{i=1,k} \rep(\phi_i, w) \label{eqn:ring:sum}
%% V2
%\rep(\boolnot \phi, w) &=& \mulident - \rep(\phi, w) \label{eqn:ring:negation} \\
...
and deletion steps, with each step preserving the solutions to
the original formula.  The status of the proof at any step is represented as
...
described in Section~\ref{sect:crat}.  Clauses can also be added or deleted based on
{\em  implication redundancy}.  That is, when clause $C$ satisfies $\phi \imply C$ for some formula $\phi$, then it can either be added to form the set $\phi \cup \{C\}$ or it can be deleted from that set to form $\phi$.
...
proof checkers~\cite{wetzler14_drattrim,RAT} for propositional logic. It provides a simple and efficient
...
significantly, we want our proofs to maintain preserve the set of solutions at
each step, whereas an unsatisfiability proof need only guarantee that
a proof step does not makes a satisfiable set of clauses become
unsatisfiable.  In particular, our proofs must justify clause deletions as well as clause additions.
...
$\pogformula = \{\{r\}\} \cup \; \bigcup_{\nodeu \in P} \theta_{u}$.  We write $\modelset(P)$ as a shorthand for $\modelset(\pogformula)$, recognizing that
...
knowledge compilers.  Although our current implementation only handles
the outputs of one particular program, we believe it could be adapted
to work with others, as well as with some model counters.
...
100~million clauses.  Compared to the problems used to assess monolithic
proof generation, this set included one that had a short proof but
