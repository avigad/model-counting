...
special form, such as a deterministic,
  decomposable, negation normal form (d-DNNF) graph.
\emph{Knowledge compilation} is the process of converting a formula
into such a form. Existing knowledge compilers provide no guarantee that their output correctly
represents the original formula, and therefore they cannot validate the model counts.
...
compilers. To capture both the structure of a POG
and a checkable proof of its logical equivalence to a Boolean formula in
conjunctive normal form (CNF), we propose the CPOG (for ``checkable'' POG) file format. A CPOG
...
We establish a set of conditions that a CPOG file must satisfy in order to
...
generates a d-DNNF representation, and the {\em proof
...
We evaluate the capabilities of this toolchain using benchmark
formulas from the 2022 standard and weighted model competitions.  Our
tools are able to handle all but the largest d-DNNF graphs generated
by \dfour{}.  We also measure the benefits of several optimizations as
well as the performance of the verified checker relative to one
written in C.
...
By contrast, our proofs construct a new propositional formula, and we must verify both that it
is partitioned and that it is equivalent to the input formula.
This requires verifying additional proof steps, including clause deletion steps, and subtle invariants, as described in
Sections~\ref{sect:cpog} and \ref{section:formally-verified-toolchain}.
...
based on the algorithms commonly used by model counters.
They present a series of lemmas and a theorem stating
...
We know of no other
work on certifying the output of a knowledge compiler.
...
\section{Generating CPOG from d-DNNF}
