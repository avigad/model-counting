...
where the formula is unsatisfiable or a tautology, we can therefore assume
...
the input formula $\inputformula$ and the POG, given by its set of defining clauses $\pogformula$.
Furthermore, it can
negate the POG formula by simply negating its root literal.  Forward implication
will hold
when $\inputformula \imply (r \land \pogformula)$.  Equivalently,
the formula $\inputformula \land \simplify{\pogformula}{\{\obar{r}\}}$
should be unsatisfiable, where the
conjunction can be expressed as the union
of the two sets of clauses.  The proof generator writes the clauses to a file, invokes a proof-generating solver, and
imports the proof steps as assertion steps in the CRAT file.
...
the size of the proof generated can be improved by two optimizations described here.
A key feature is that they do not require any changes to the
...
variable $v$.  A single call to the solver can generate this proof.
Looking at the defining clauses for $\nodev$ (Table
...
detecting that the simplified set of
...
$\nodeu$ as a child.  The net effect is that generator will recurse during a call to $\validate$ only once for each node in the POG\@.
