...
required by \dfour{} (X axis) versus the sum of the times for proof generation, checking, and counting
 (Y axis).
...
generator, checker, and counter.  These were for problems having small numbers
...
check it (and count).  This benchmark appears to stem from weak performance
...
mode on the 92 problems for which the combination of structural proof generation, checking, and counting
required at most 3000 seconds and then kept only those
measurements for which at least one of the two approaches had combined
times of at most 1000 seconds.  There were
...
the problems, including a case where the generator, checker, and counter
...
reduced by at least a factor of 2 by using lemmas, 14 by using literal merging,
...
assumptions about how the compiled representation was generated.  A complete
...
entire toolchains: one with compilation, proof generation, 
checking, and counting, and the other starting with preprocessing and then having
...
This section provides more insights into how our proof framework (CPOG)
...
generation, proof checking, and counting in the CPOG toolchain compares to the time to
