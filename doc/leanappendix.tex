\section{Detailed Information about the Formally Verified Checker}
\label{appendix:lean}
...
and proof assistant~\cite{demoura:cade:2021}. \lean{} implements a logical foundation in which expressions
...
Verifying our checker and the model counters in this way serves at
least two purposes.  When we run them on particular data sets, they
provide high confidence that the particular results we obtain are
correct.  More broadly, however, our success also validates our
general approach, showing, in particular, that the CRAT proof format
is sound.  Indeed, our formalization helped us discover a bug in our
prototype proof checker: the declaration a sum operation via the
``\texttt{s}'' step (Section~\ref{subsection:syntax}) requires a hint
consisting of a sequence of clause Ids that provide a RUP proof of the
mutual exclusion condition $\obar{\lit}_1 \lor \obar{\lit}_2$.  It is
sound (and necessary) to allow these Ids to be those of defining
clauses for previous POG nodes, but, as we discovered, it is generally
not sound to allow them to be for input clauses or other added
clauses.  We therefore added the condition that only defining clauses
could be included in the hint, and we implemented a test of this rule
in our prototype checker.  Our verified implementation of the checker
tells us that the algorithm is sound and provides a reference for
...
% This didn't fit in the main section:
\iffalse
